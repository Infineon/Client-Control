    ...
    m_nvram(nullptr), m_nvram_id(-1), m_paired(paired)
{
    ...
    m_app_status_ready = false;

    memset(source_supported_config, 0, sizeof(source_supported_config));
    memset(sink_supported_config, 0, sizeof(sink_supported_config));
}
...
void CBtDevice::set_app_ready_status(bool ready)
{
    m_app_status_ready = ready;
}
bool CBtDevice::is_app_status_ready()
{
    return m_app_status_ready;
}
...
void CBtDevice::le_audio_handle_connection(void)
{
    memset(source_supported_config, 0, sizeof(source_supported_config));
    memset(sink_supported_config, 0, sizeof(sink_supported_config));
    memset(&le_audio_state, 0, sizeof(le_audio_state));
}
...
void MainWindow::EnableUI(bool bEnable)
{
    ...
    ui->checkBoxFilterAdv->setEnabled(bEnable);
...
void MainWindow::EnableTabs(UINT8 feature, bool bEnable)
{
    if((feature != 0xFF) && bEnable)
    {
        ...
        case HCI_CONTROL_GROUP_RAS:
            ui->tabRas->setEnabled(bEnable);
            ui->tabMain->setCurrentWidget(ui->tabRas);
            break;
        }
    }
    else
...
void MainWindow::ReadDevicesFromSettings(const char *group, QComboBox *cbDevices, QPushButton *btnUnbond)
{
    ...
    for (int i = 0; i < device_keys.size(); i++)
    {
        ...
        QString dev_name_id = m_settings.value(device_keys[i],"").toString();

        if (dev_name_id.size())
        {
            QStringList parts = dev_name_id.split(":");
            if(parts.length() > 0)
            {
                sscanf(parts[0].toStdString().c_str(),"%04x", &nvram_id);
            }
            if(parts.length() > 1){
                dev_name = parts[1];
            }
...
void MainWindow::onUnbond(QComboBox* cb)
{
    ...
    sprintfBDA(strBda, pDev->m_address);
...
void MainWindow::onLEDevChange(QString)
{
    ...
    if (pDev)
    {
        ...
        {
           le_audio_handle_device_change(pDev);
        }
        // switch to BLE
...
void MainWindow::OnBnClickedDiscoverDevicesStart()
{
    if (!m_scan_active)
    {
        ...
        app_host_dm_le_scan(true, ui->checkBoxFilterAdv->isChecked());
    }
...
void MainWindow::OnBnClickedDiscoverDevicesStop()
{
    if (m_scan_active)
    {
        ...
        app_host_dm_le_scan(false, true);
    }
    setGATTUI();
...
CBtDevice *MainWindow::FindInList(BYTE * addr, QComboBox * pCb)
{
    ...
    CHAR bda_string[40];
    get_bd_string(addr, bda_string, sizeof(bda_string));
    ...
    for (int i = 0; i < pCb->count(); i++)
    {
        ...
        //CHAR trace[256];
        //sprintf (trace, "FindInList addr %s con_handle = %d, m_paired = %d, m_address %s",bda_string,
...
void MainWindow::HandleDeviceEventsDm(DWORD opcode, LPBYTE p_data, DWORD len)
{
    ...
    case HCI_CONTROL_EVENT_IDENTITY_KEYS:
        m_settings.beginGroup("Local keys");
        if(m_settings.value("IRK ID:", 0).toUInt() == 0)
        {
            uint16_t nvram_id;
            STREAM_TO_UINT16(nvram_id, p_data);
            char * pc = reinterpret_cast<char *>(p_data);
            QByteArray val (const_cast<const char *>(pc), static_cast<int>(len - 2));
            Log("HCI_CONTROL_EVENT_IDENTITY_KEYS %d", nvram_id);
            m_settings.setValue("IRK ID:", nvram_id);
            m_settings.setValue("IRK:", val);
        }
        m_settings.endGroup();
        break;
    ...
    case HCI_CONTROL_EVENT_NVRAM_DATA:
    {
        ...
        m_settings.endGroup();
        m_settings.sync();
        memcpy(bd_addr, &p_data[2], BD_ADDR_LEN);
        ...
        if ((len - 2) > 65)
        {
            ...
            int bleindex = -1;
            int brindex = -1;
            sprintfBDA(abuffer, bd_addr);

            bleindex = ui->cbBLEDeviceList->findText( abuffer,Qt::MatchStartsWith);
            brindex = ui->cbDeviceList->findText( abuffer,Qt::MatchStartsWith);

            if(bleindex != -1)
            {
                device = static_cast<CBtDevice *>(ui->cbBLEDeviceList->itemData(bleindex).value<CBtDevice*>());
                if(bLeDevice && (memcmp(device->m_address, bd_addr, 6) == 0))
                {
            ...
            if(brindex != -1)
            {
                ...
                if(brdevice && (memcmp(brdevice->m_address, bd_addr, 6) == 0))
                {
                    needSetIdentityAddr = 0;
                }
            }

            if(needSetIdentityAddr && device)
            {
                memcpy(device->m_address, bd_addr, 6);
                device->address_type = p_data[26];
                abuffer.sprintf("%02x:%02x:%02x:%02x:%02x:%02x (%s)",
                        bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5], device->m_name);

                ui->cbBLEDeviceList->setItemText(bleindex, abuffer);
            }

            CBtDevice * pDev = AddDeviceToList(bd_addr, bLeDevice ? ui->cbBLEDeviceList : ui->cbDeviceList, nullptr, true);
...
const name_val_t misc_app_status[] = {
    {"connected", HCI_CONTROL_MISC_APP_STATE_CONNECTED},
    {"mtu_configured",HCI_CONTROL_MISC_APP_STATE_MTU_CONFIGURED},
    {"discovery_complete",HCI_CONTROL_MISC_APP_STATE_DISCOVERY_COMPLETE},
    {"init", HCI_CONTROL_MISC_APP_STATE_INITING},
    {"ready",HCI_CONTROL_MISC_APP_STATE_READY},
    {"disconnecting", HCI_CONTROL_MISC_APP_STATE_DISCONNECTING},
    {"disconnected", HCI_CONTROL_MISC_APP_STATE_DISCONNECTED}
};

const name_val_t misc_app_sub_state[] = {
    {"notifying", HCI_CONTROL_MISC_APP_STATE_INIT_NOTIFYING},
    {"enabling",HCI_CONTROL_MISC_APP_STATE_INIT_ENABLING},
    {"reading",HCI_CONTROL_MISC_APP_STATE_INIT_READING}
};

#define TAG "Misc"
void MainWindow::misc_handle_app_status_events(BYTE *p_data, DWORD len)
{
    uint16_t handle = -1;
    ...
    if(len >= 10){
        ...
        p_data += ReadBDAFromStream(bd_addr, p_data);
    ...
    CBtDevice *p_device = (app_status == HCI_CONTROL_MISC_APP_STATE_CONNECTED) ? FindInList(bd_addr, ui->cbBLEDeviceList) : FindInList(CONNECTION_TYPE_LE, handle, ui->cbBLEDeviceList);
    if(p_device)
        device_addr_str = p_device->get_bdaddr_string();
    app_status_str = get_name_for_val(misc_app_status, sizeof(misc_app_status)/sizeof(misc_app_status[0]), app_status, "unknown");

    switch(app_status)
    {
    case HCI_CONTROL_MISC_APP_STATE_CONNECTED:
        if(GetSelectedLEDevice() == p_device)
            ui->btn_le_audio_connectToPeer->setText(STRING_DISCONNECT);

        /* mark device not ready */
        if(p_device)
        {
            p_device->set_app_ready_status(0);
        }
        Log("%s app status %s %s 0x%x", TAG, device_addr_str, app_status_str, sub_status);
        break;
    case HCI_CONTROL_MISC_APP_STATE_DISCONNECTED:
        if(GetSelectedLEDevice() == p_device)
            ui->btn_le_audio_connectToPeer->setText(STRING_CONNECT);
        Log("%s app status %s %s 0x%x", TAG, device_addr_str, app_status_str, sub_status);
        break;
    case HCI_CONTROL_MISC_APP_STATE_MTU_CONFIGURED:
    case HCI_CONTROL_MISC_APP_STATE_DISCOVERY_COMPLETE:
    case HCI_CONTROL_MISC_APP_STATE_DISCONNECTING:
        Log("%s app status %s %s 0x%x", TAG, device_addr_str, app_status_str, sub_status);
        break;
    case HCI_CONTROL_MISC_APP_STATE_READY:
        Log("%s app status %s %s 0x%x", TAG, device_addr_str, app_status_str, sub_status);
        if(p_device){
            p_device->set_app_ready_status(1);
        }
    break;
    case HCI_CONTROL_MISC_APP_STATE_INITING:
        sub_status_str = get_name_for_val(misc_app_sub_state, sizeof(misc_app_sub_state)/sizeof(misc_app_sub_state[0]), sub_status, "unknown");
        Log("%s app status %s %s %s 0x%04x", TAG, device_addr_str, app_status_str, sub_status_str, uuid_32);
        break;
    case HCI_CONTROL_MISC_APP_STATE_BONDING_COMPLETED:
        if(p_device)
        {
            QString abuffer;
            sprintfBDA(abuffer, p_device->m_address);
            int index = ui->cbBLEDeviceList->findText(abuffer, Qt::MatchStartsWith);
            memcpy(p_device->m_address, bd_addr, BD_ADDR_LEN);
            p_device->m_paired = true;
            abuffer.sprintf("%02x:%02x:%02x:%02x:%02x:%02x (%s)",
                                    bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5], p_device->m_name);

            ui->cbBLEDeviceList->setItemText(index, abuffer);
            ui->cbBLEDeviceList->setItemIcon(index, m_paired_icon);
        }
        Log("%s app status %s %s 0x%x", TAG, device_addr_str, app_status_str, sub_status);
        break;
    }
...
void MainWindow::HandleDeviceEventsMisc(DWORD opcode, LPBYTE tx_buf, DWORD len)
{
    ...
    case HCI_CONTROL_MISC_EVENT_VERSION:
    {
        ...
        for(; index < len; index++)
        {
            // Log("\t%d tab %d",index - 9, tx_buf[index]);
    ...
    case HCI_CONTROL_MISC_EVENT_APP_STATUS:
        misc_handle_app_status_events(tx_buf, len);
        break;
    }
}
...
void MainWindow::on_btnOpenPort_clicked()
{
    ...
    if(!m_bPortOpen)
    {
        ...
        else
        {
            ...
            Log("[%s] LE device count %d ",__FUNCTION__, ui->cbBLEDeviceList->count());
            for (int dev_idx = 0 ; dev_idx < ui->cbBLEDeviceList->count() ; dev_idx++)
            {
                QVariant var = ui->cbBLEDeviceList->itemData(dev_idx);
                CBtDevice * pDev = var.value<CBtDevice *>();

                Log("[%s] %p",__FUNCTION__, pDev);
                if(pDev != nullptr)
                {
                    Log("[%s] %s",__FUNCTION__, pDev->get_bdaddr_string());
                    pDev->set_connection_in_progress(FALSE);
                    pDev->set_connection_handle(0);
                }
            }
...
DWORD Worker::ReadNewHciPacket(BYTE * pu8Buffer, int bufLen, int * pOffset)
{
    ...
    {
        dwLen = Read(&pu8Buffer[offset], 3);
        if(dwLen == 3)
        {
            len = static_cast<DWORD>(pu8Buffer[3]); // data_length is 1 byte
            offset += 3;
        }
        else
            m_pParent->Log("error HCI_SCO_DATA_PKT needed 3 got %ld", dwLen);
    }
        break;
    case HCI_ISO_DATA_PKT:
    {
        ...
        if(dwLen == 4)
        {
            len = static_cast<DWORD>(pu8Buffer[3] | ((pu8Buffer[4] & 0x3F) << 8));
            offset += 4;
            m_pParent->Log("HCI_ISO_DATA_PKT, len %ld", len);
        }
        else
            m_pParent->Log("error HCI_ISO_DATA_PKT needed 4 got %ld", dwLen);
    }
        break;
...
void MainWindow::Startup()
{
    ...
    // Send stored identity resolving key
    m_settings.beginGroup("Local keys");
    if(m_settings.value("IRK ID:", 0) != 0)
    {
        uint16_t nvram_id = m_settings.value("IRK ID:").toUInt();
        QByteArray irk = m_settings.value("IRK:").toByteArray();
        char * nvdata = const_cast<char *>(irk.constData());
        app_host_dm_push_nvram_data(nvram_id, reinterpret_cast<unsigned char*>(nvdata), irk.size());
    }
    m_settings.endGroup();
