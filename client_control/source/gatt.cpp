...
void MainWindow::OnBnClickedStartStopAdvertisements()
{
    ...
    app_host_gatt_start_stop_advert(m_advertisments_active, 0, 0);
}
...
void MainWindow::on_LeConnectPressed(QString connText)
{
    ...
    if (connText == STRING_CONNECT)
    {
        if((p_device->get_connection_handle() == 0) && (p_device->connect_in_progress() == 0)){
            ...
            p_device->set_connection_in_progress(1);
            app_host_gatt_connect(p_device->address_type, p_device->m_address);
        }else{
...
int MainWindow::ReadBDAFromStream(uint8_t *p_dest, uint8_t *p_src)
{
    int i;
    for (i = 0; i < 6; i++)
        p_dest[5 - i] = p_src[i];
...
void MainWindow::sprintfBDA(QString &bda, uint8_t *p_bda)
{
    bda.sprintf("%02x:%02x:%02x:%02x:%02x:%02x", p_bda[0], p_bda[1], p_bda[2], p_bda[3], p_bda[4], p_bda[5]);
}
...
void MainWindow::HandleLEEvents(DWORD identifier, LPBYTE p_data, DWORD len)
{
    ...
    case HCI_CONTROL_LE_EVENT_EXT_ADVERTISEMENT_REPORT:
    {
        char bd_name[50] = {0};
	    uint8_t addr_type = p_data[1];

	    if(identifier == HCI_CONTROL_LE_EVENT_EXT_ADVERTISEMENT_REPORT){
            addr_type = p_data[8];
	    }

        ReadBDAFromStream(bda, &p_data[2]);
        ...
        sprintf (trace, "Advertisement report:%d type:%d address %02x:%02x:%02x:%02x:%02x:%02x rssi:%d",
            p_data[0], addr_type, p_data[7], p_data[6], p_data[5], p_data[4], p_data[3], p_data[2],
            p_data[8] - 256);
        Log(trace);

        DWORD start_adv = 9;
        //DWORD adv_len = p_data[start_adv];

        if(identifier == HCI_CONTROL_LE_EVENT_EXT_ADVERTISEMENT_REPORT){
            start_adv = 25;
            //adv_len = len - start_adv;
        }

        // dump advertisement data to the trace
        if (len > start_adv)
        {
            DecodeEIR(&p_data[start_adv], len - start_adv, bd_name, sizeof(bd_name));
#if 0
            DWORD used = 0;
            trace[0] = 0;
            for (DWORD i = 0; i < len - start_adv; i++)
                used += snprintf(trace + used, sizeof(trace) - used, "%02x", p_data[start_adv + i]);
            Log(trace);
    ...
    case HCI_CONTROL_LE_EVENT_ADVERTISEMENT_STATE:
        ...
        break;
    case HCI_CONTROL_LE_EVENT_EXT_ADVERTISEMENT_STATE:
        {
            uint32_t adv_state;
            STREAM_TO_UINT32(adv_state, p_data);

            Log(" adv state %d",  adv_state);
            if(adv_state == LE_EXT_ADV_STATE_IDLE)
                ui->btn_le_audio_startLeAdv->setText("Start Adv");
            else
                ui->btn_le_audio_startLeAdv->setText("Stop Adv");
        }
        break;
    case HCI_CONTROL_LE_EVENT_CONNECTED:
    {
        int connected = p_data[10];
        sprintf (trace, "Connection %s:type:%d address %02x:%02x:%02x:%02x:%02x:%02x connection handle:%04x role:%d",
                 p_data[10] ? "up":"down",
        ...
        ReadBDAFromStream(bda, &p_data[1]);
        ...
        if(connected)
        {
            ...
            device->le_audio_handle_connection();
    ...
    case HCI_CONTROL_LE_EVENT_DISCONNECTED:
    {
        ...
        tlen = snprintf (trace + tlen, sizeof(trace) - tlen,
            "Connection down:connection handle:%04x reason:0x%x", p_data[0] + (p_data[1] << 8), p_data[2]);
        if(len >= 10)
        {
            /* Disconnect event contains the bdaddr from 4:9*/
            ReadBDAFromStream(bda, &p_data[4]);
            tlen = snprintf (trace + tlen, sizeof(trace) - tlen, " address ");
            tlen += get_bd_string(bda, trace + tlen, sizeof(trace) - tlen);
            has_bdaddr = 1;
        }
        ...
        if(!device && has_bdaddr)
        {
            device = FindInList(bda, ui->cbBLEDeviceList);
        }
